...
7 PRINTM$;:CC=((PEEK(384)AND1)*256+PEEK(385))-1+MC:FORC1=CC-LEN(M$)+1TOCC:POKEC1,PEEK(C1)-64:NEXT:RETURN
10 DIM PL(13),CO(13),DE(13),GU(13),PO(13),AS(13),SC(1),VI(1),PA(2),DR(2)
20 CLS:PRINT "GO FISH":PRINT "ŒŒŒŒŒŒŒ":PRINTTAB(6)"Š":PRINTTAB(6)"ŠŽŒŽ":PRINTTAB(6)"ŠŒ…†€†"
...
654 IFPD<1THENPD=13
655 K=PD:IFAS(K)THENGOSUB995:IFC1>1THEN650
660 IFCO(K)=0THEN650
670 REM MAKE TURN
...
980 NEXT I
990 RETURN
995 C1=0:FORZX=1TO13:IFCO(ZX)>0THENC1=C1+1
996 NEXT:RETURN
...
9010 NEXT:RETURN
9100 REM FROM LOCOMOTIVE BASIC
9110 REM ROSETTACODE.ORG 2014
