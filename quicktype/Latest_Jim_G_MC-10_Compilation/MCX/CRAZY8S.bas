0 CLS:CLEAR500:DIMZZ,CC,C3,M$,MC,C1$(4),C2$(4),CL(8):MC=16384:GOSUB3000:ZZ=RND(-(PEEK(9)*256+PEEK(10))):GOTO100
1 ZZ=1:CC=32
2 IFMID$(M$,CC,1)<>""ANDMID$(M$,CC,1)<>" "THENCC=CC-1:GOTO2
3 PRINTMID$(M$,ZZ,CC-ZZ):ZZ=CC+1:CC=ZZ+31:IFZZ<=LEN(M$)THEN2
4 RETURN
6 FORZZ=CC+MCTOC3+MC:POKEZZ,PEEK(ZZ)-64:NEXT:RETURN
7 CC=(PEEK(17024)AND1)*256+PEEK(17025):PRINTM$;:C3=((PEEK(17024)AND1)*256+PEEK(17025))-1:GOSUB6:RETURN
10 IFCT<8THENPRINT
11 FOR CG=1 TO CT:PRINT C2$(CL(CG));:NEXT CG:CN=0:IFCT<8THENPRINT
12 RETURN
...
230 DATA "2","3","4","5","6","7","8","9",TEN,JACK,QUEEN,KING,ACE   
...
320 PRINT "TAKE THE FORM: cARD,sUIT.       ANY EIGHT MAY BE PLAYED AT"
330 PRINT "ANY TIME TO CHANGE THE SUIT."   
340 PRINT "IF YOU ARE UNABLE OR UNWILLING  TO PLAY, YOU WILL BE GIVEN"   
350 PRINT "ANOTHER CARD FROM THE STACK.    IF ALL CARDS HAVE BEEN DRAWN,
360 PRINT "ANY PERSON WHO CAN PLAY MUST    DO SO. AT THE END OF THE GAME,
370 PRINT "THE PERSON STILL HOLDING CARDS  HAS POINTS SCORED AGAINST HIM:"
380 PRINT "EIGHTS=50, ACE=1, FACE CARD=10, INDEX VALUE FOR EACH OTHER CARD.";   
390 M$="PRESS ANY KEY TO CONTINUE":GOSUB7
395 I$=INKEY$:ON1-(I$<>"")GOTO395:CLS
...
1050 PRINT   
1060 M$="OUT OF"+STR$(N3)+" HANDS YOU WON"+STR$(N1)+".":GOSUB1
1070 M$="YOUR TOTAL SCORE IS"+STR$(S1)+".":GOSUB1:M$="MY TOTAL SCORE IS"+STR$(S2)+".":GOSUB1
1080 IF S1<=S2 THEN1110   
1090 PRINT "LOOKS LIKE YOU'RE HIGH SCORER."  
1100 GOTO 1120
1110 PRINT "LOOKS LIKE I'M THE HIGH SCORER."
1120 PRINT "BYE."
1130 END   
1140 C1=0   
1150 C2=0   
...
1520 INPUT A$   
1530 AZ$=LEFT$(A$,1):IF AZ$="Y" THEN 1630
...
1590 H1=H1+1
1600 G(I)=1 
1610 GOSUB5000:GOSUB6000
1620 GOTO 1510  
1630 PRINT "WHICH CARD";   
1640 INPUT X$,Y$
1650 GOSUB 2610 
1660 IF V9>0 THEN 1680
1661 PRINT "UNKNOWN CARD, TRY AGAIN";
1670 GOTO 1640 
1680 GOSUB 2500 
1690 IF S9>0 THEN 1710
1691 PRINT "UNKNOWN SUIT, TRY AGAIN";
1700 GOTO 1640 
...
1890 INPUT Y$
1900 GOSUB 2500 
1910 IF S9>0 THEN 1930
1911 PRINT "UNKNOWN SUIT, TRY AGAIN";
1920 GOTO 1890 
1930 P=S9   
1940 P1=1   
...
3060 C1$(4)="¡« "
3070 C2$(4)=" ¬®¨":RETURN
5000 PRINT "YOUR HAND IS"   
5410 CN=0:FOR I=1 TO 52  
5420 IF G(I)=0 THEN 5440
5430 CN=CN+1:PRINT LEFT$(C$(V(I)),1);C1$(S(I));:CL(CN)=S(I):IFCN=8THENCT=8:GOSUB10
5440 NEXT I:IFCN>0THENCT=CN:GOSUB10
5450 RETURN
5500 FOR I=1 TO 52  
5510 IF H(I)=0 THEN 5540
5520 CN=CN+1:PRINT LEFT$(C$(V(I)),1);C1$(S(I));:CL(CN)=S(I):IFCN=8THENCT=8:GOSUB10
5530 C1=C1+Z(I) 
5540 NEXT I:IFCN>0THENCT=CN:GOSUB10
5550 RETURN
6000 PRINT "LAST CARD PLAYED WAS" 
6010 PRINT LEFT$(C$(V(T(T1))),1);C1$(S(T(T1)))
6020 PRINT C2$(S(T(T1)))
6030 IF V(T(T1))<>7 THEN 6050
6040 M$="THE SUIT CALLED FOR IS "+U$(P):GOSUB1
6050 RETURN
