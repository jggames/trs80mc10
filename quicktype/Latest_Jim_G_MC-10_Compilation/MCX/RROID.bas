...
11 PRINT@P(H),D$(H);:PRINT@P(H)+A,E$(H);:PRINT@P(H)+B,F$(H);:PRINT@P(H),A$(H);:PRINT@P(H)+A,B$(H);:PRINT@P(H)+B,C$(H);:SOUND100,1:RETURN
...
19 GOTO60
20 FORE=1TO1000:FORT=1TOZ:O=P(T):X(T)=X(T)+A(T,X(T)):Y(T)=Y(T)+B(T,Y(T)):P(T)=A*INT(Y(T))+X(T):PRINT@O,D$(T);:PRINT@O+A,E$(T);
22 PRINT@O+B,F$(T);:PRINT@P(T),A$(T);:PRINT@P(T)+A,B$(T);:PRINT@P(T)+B,C$(T);:ONPOINT(S(X),T(Y))GOSUB45,45,45,45,45,45,45,45
24 ONK(PEEK(383)ANDPEEK(2))GOSUB4,5,6,13,31:PRINT@P,S$(D);:PRINT@P+A,T$(D);:ONFGOSUB14,40,40,40,40,40:NEXT:NEXT
...
810 DIMX$(8,4),Y$(8,4),Z$(8,4),N(2),T,O,P,D,A,B,X,Y,C,E,F,H,I,J,L,M,Q,R,Z,S,SH,SC,HS,LV,LC,LM,LN,TM,B$,S$,T$,I$
...
1000 N(1)=-1:N(2)=1
1010 K(83)=1:K(65)=2:K(87)=3:K(32)=4:K(66)=5
