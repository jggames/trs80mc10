...
290 ST=4:GOSUB1420
300 PRINT@128,STR$(INT(L))","INT(M);:PRINT@192,STR$(INT(U))","INT(V);
...
1260 CD=INT(SQR(X^2+A^2)):PRINT@352,"COURSE ="LEFT$(STR$(CC),5)", DISTANCE ="LEFT$(STR$(CD),5):GOTO1110
1270 NEXT:GOTO1110
1280 IFD(5)<0THEN370
1281 IFAF=0THEN1300
...
1286 GOTO880
1290 AF=0:IFCD<>0THENC=CC:W=CD:GOTO470
...
1400 PRINT@121,INT(E):GOSUB1420 
1410 PRINT@185,INT(G):GOSUB1500:IFG<0THEN1070
1411 RETURN 
...
1480 FORFL=1TO4:PRINT@57,C$(1);:FORZZ=1TO75:NEXT:PRINT@57,C$(ST);:FORZZ=100TO50STEP-4:SOUNDZZ,1:NEXTZZ
...
1581 GOSUB1600:FORZ=1TO20:SOUNDRND(9)*9,1:PRINT@Q+RND(2)-2,CHR$(128+RND(62));:NEXT
...
1680 AL=AL+1:ONALGOTO1681,1681,1681,1681,1681,1681,1681,1681,1682,1683,1683,1683,1683,1683,1683,1683,1683,1683,1683,1684
1681 PRINT@57,C$(1);:RETURN
1682 SOUND50,1:RETURN
1683 PRINT@57,C$(ST);:RETURN
1684 AL=0:RETURN
1690 SOUND25,1:FORZO=5TO3STEP-1:FORZN=12TO1STEP-1:SOUNDZO,1:NEXTZN,ZO:RETURN
1700 FORX=50TO70STEP5:SOUNDX,1:NEXT:RETURN
2000 REM MC-10 EDITS JIM GERRIE
