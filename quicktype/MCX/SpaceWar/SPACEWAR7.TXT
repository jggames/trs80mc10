...
80 REM*                 *
90 REM*******************
100 DIMC(46),O(21,3),S(16,2),E(8,2),X,Y,M,I,J,N,R1,R2,R3,R4,R5,R6,A,B,U,U1,U2,V,V1,V2,E,F,G,F1,F2,F3,Z,P,CX,CY,P1,P2,S
...
160 GOSUB2740:G=R1
170 M=RND(-(PEEK(9)*256+PEEK(10))):REM CHANGE TO M=RND(-TIMER) FOR COCO
...
300 INPUTA$:P1$=A$
310 GOSUB2000:ONR1 GOTO290,290,290,270,290,2980
320 IFA$=""THENA$=P$
330 Q=VAL(A$)
...
420 INPUT"NAME";A$
430 P1$=A$:GOSUB2000:ONR1 GOTO280,440,280,270,280,2980
440 IFA$=""THEN460
450 P1$=A$:GOSUB2610:N$(J)=R1$
460 NEXTJ
470 PRINT"(";N1$;") # OF STARS? (0-18)";
480 INPUTA$
490 P1$=A$:GOSUB2000:ONR1 GOTO280,470,470,470,280,2980
500 IFA$=""THENA$=N1$
510 N=VAL(A$)
530 IFABS(N)<0ORABS(N)>18THEN470
540 N1$=STR$(N)
550 REM O=BLACKHOLE
560 O=-(N<0)
570 N=ABS(N)
...
640 R=0
650 R=R+1
660 IFR>20THENI=N:NEXTI:GOTO630
670 O(I,3)=Q(4)
680 REM--J:1=XPOS,2=YPOS,3=MASS/SIZE--
690 FORJ=3TO1STEP-1:O(I,J)=INT((Q(J)-2*O(I,3))*RND(0)+O(I,3)):NEXTJ:IFI=1THEN760
720 FORJ=1TOI-1:T=(O(I,3)+O(J,3)+10)^2:IF(O(I,1)-O(J,1))^2+(O(I,2)-O(J,2))^2<T THENJ=I:NEXTJ:GOTO650
750 NEXTJ
760 NEXTI
770 REM---PLACE SPACESHIPS (SIZE=5)---
780 O(N+1,3)=5:O(N+2,3)=5
800 FORJ=1TO2:T=4*LEN(N$(J)):IFT<10THENT=10
820 IFJ=1THENI=.28*Q(1):Q=0:GOTO840
830 T=-T:I=-.28*Q(1):Q=Q(1)
840 R=0
850 R=R+1
860 IFR>20THENF1=J:J=2:NEXTJ:J=F1:GOTO630
870 X=INT(I*RND(0))+Q+T:Y=INT((Q(2)-30)*RND(0)+20)
890 FORK=1TON:IF(X-O(K,1))^2+(Y-O(K,2))^2<(O(K,3)*2)^2THENF1=K:K=N:NEXTK:K=F1:GOTO850
910 NEXTK:O(N+J,1)=X:O(N+J,2)=Y:NEXTJ
950 A1$(1)="0":A1$(2)="180"
...
1030 FORI=1TON-O:FORJ=1TO37:X=INT(O(I,1)+O(I,3)*C(J)+.5):Y=INT(O(I,2)-O(I,3)*C(J+9)+.5):IFJ=1THENLINE(X,Y)-(X,Y),PSET
...
1525 SCREEN2:U1=INT(U+.5):V1=INT(V+.5):IFU1<>U2ORV1<>V2 THENP1=U1:P2=V1:GOSUB3
1550 U2=U1:V2=V1
1570 E=0:F=0
1580 FORJ=1TON:R5=O(J,1)-U:R6=O(J,2)-V:R4=R5^2+R6^2:R3=SQR(R4):R2=R5/R3:R1=R6/R3:G=.017*(O(J,3)^3)/R4:E=E+G*R2:F=F+G*R1:NEXT
1690 X=X+E:Y=Y+F:U=U+X:V=V+Y:IFF2 THEN1790
1750 FORJ=1TON+2:IFO ANDJ=N THEN1780
1770 IF(U-O(J,1))^2+(V-O(J,2))^2<O(J,3)^2THENF1=J:J=N+2:NEXTJ:J=F1:GOTO1890
1780 NEXTJ
1790 IFU<-Q(1)ORU>Q(1)*2ORV<-Q(2)ORV>Q(2)*2THEN1200
1800 F2=(U<0ORU>Q(1)ORV<0ORV>Q(2))
1840 IFF2=F3 THEN1510
1850 IFF3 THENP1=U:P2=V:GOSUB2230
1860 F3=F2:GOTO1510
1880 REM---MAKE EXPLOSIONS---
1890 P1=U+2:P2=V+2:GOSUB2230
1900 FORI=1TO8:P1=E(I,1):P2=E(I,2):GOSUB1:NEXT
1910 IFJ<=N THEN1200
1920 SOUND100,10:SOUND200,10
1921 SCREEN2:SOUND1,1
...
1990 GOTO270
2000 REM***INPUT*FNC***
2010 P1$=LEFT$(P1$,1):IFP1$=""THENR1=0:RETURN
2061 P=7
...
2440 PRINT"H=HELP"
2450 PRINT"S=START"
2460 PRINT"D=DISPLAY (CLEARS TRACKS)"
...
2690 IFM THENP1$=MID$(P1$,1,I-1)+CHR$(K+32)+MID$(P1$,I+1)
2700 M=1
2710 NEXTI:R1$=P1$:RETURN
2740 REM***POLL TERMINAL CAPABILITY*FNI***
2750 R1=1:Q(1)=255:Q(2)=191:RETURN
2970 REM***EXIT***
2980 CLS
2990 END
